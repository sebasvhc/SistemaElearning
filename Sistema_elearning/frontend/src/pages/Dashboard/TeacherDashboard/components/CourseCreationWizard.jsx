...
const CourseCreationWizard = ({ periods, teacherId, onSuccess, onCancel }) => {
    ...
    // Función validateStep única y actualizada
    const validateStep = (step) => {
        ...
        if (step === 1) {
            if (courseData.objectives.length === 0) {
                ...
                const totalWeight = courseData.objectives.reduce(
                    (sum, obj) => sum + (parseInt(obj.weight) || 0), 0
                );
                if (totalWeight !== 100) {
                    newErrors.objectives = `La suma total debe ser exactamente 100% (actual: ${totalWeight}%)`;
                }
            }
        }
    ...
    const updateCourseData = (field, value) => {
        ...
        // Limpiar error específico al actualizar el campo
        if (errors[field]) {
